...
import { signal, computed } from "@vanillakit/signal";
...
describe("Each() keyed lists", () => {
  test("renders initial list", () => {
    ...
    const node = html`<ul>${Each({list: items, key: (t) => t.id}, (item) => html`<li>${() => item().text}</li>`)}</ul>`;
    document.body.append(node);
  ...
  test("index signal updates when items reorder", () => {
    ...
    const indexValues = [];
    const node = html`<ul>${Each({list: items, key: (t) => t.id}, (item, idx) => {
